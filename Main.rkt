...
(define (Stat_to_String_5 x)
  (string-append "Average Number of lands in deck with this card: " (number->string x)  ))

(define (Stat_to_String_6 x)
  (string-append "Average Converted Mana Cost in decks with this card: " (number->string x) ))


(define (Card-Name-Converter string)
  (string-downcase string)
  (string-append "img/" string ".hq.jpg"))
...
(define (view string) 
  ...
  (define tCard string)
  (define img (Card-Name-Converter string))
(place-images
   (list (text tCard 60 "black") ;;; Card Name
         (text (Stat_to_String_1 (ask database 'decks-containg tCard) ) 20 (Color_Coder (ask database 'decks-containg tCard))) ;;; Stat 1
         (text (Stat_to_String_2 (ask database 'total-copies tCard) ) 20 (Color_Coder (ask database 'total-copies tCard))) ;;; Stat 2
         (text (Stat_to_String_3 (ask database 'maindeck-copies tCard) ) 20 (Color_Coder (ask database 'maindeck-copies tCard))) ;;; Stat 3
         (text (Stat_to_String_4 (ask database 'sideboard-copies tCard) ) 20 (Color_Coder (ask database 'sideboard-copies tCard))) ;;; Stat 4
         (text (Stat_to_String_6 (exact->inexact (ask database 'avg-cmc-by-card tCard)) ) 20 (Color_Coder (ask database 'avg-cmc-by-card tCard))) ;;; Stat 5
         (text (Stat_to_String_5 (exact->inexact (ask database 'avg-land-by-card tCard)) ) 20 (Color_Coder (ask database 'avg-land-by-card tCard))) ;;; Stat 6
         (bitmap/file img ) ;;; Card Image
         (text "Gregory James Caldwell Jr. & Robert Shaffer" 10 "firebrick") ;;; Names
         (star 10 "solid" "firebrick")) ;;; Misc Water Mark
   
   (list (make-posn 290 30) ;;; Card Name Position 
         ...
         (make-posn 255 350)  ;;; Stat 5 Position 
         (make-posn 245 400)  ;;; Stat 6 Position 
         (make-posn (- 800 127.5 ) 600)  ;;; Card Image
         (make-posn 125 790)  ;; Watermark Names Position 
         (make-posn 10 790)) ;; Watermark Star Position
   (rectangle 1000 850 "solid" "darkcyan"))
  )

;; Predfined Test Cards. 
(define T1 (view "Abzan Charm"))
(define T2 (view "Jungle Hollow"))
(define T3 (view "Sylvan Caryatid"))
(define T4 (view "Courser of Kruphix"))
(define T5 (view "Hornet Queen"))